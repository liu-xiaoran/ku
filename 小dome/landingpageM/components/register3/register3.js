...
module.exports = {
    ...
    render: function (dom) {
        ...
        // 择
        var tpl = __inline('register3.handlebars');

	    // 模板数据
	    var data = {
	        // 使用__uri函数来定位任意工程文件，scrat构建之后，会
	        // 将其替换为发布地址，这样工程就不用关心部署相关问题了
	        bgimg: window.remoteConfig.register3_bgimg,
	        img: window.remoteConfig.register3_src,
	        color1: window.remoteConfig.register3_color1,
	        color2: window.remoteConfig.register3_color2,
	        color3: window.remoteConfig.register3_color3,
	        color4: window.remoteConfig.register3_color4,
	        color5: window.remoteConfig.register3_color5
	    };
	    // 使用模板+数据得到html
	    dom.innerHTML = tpl(data);
//      dom.innerHTML = __inline('register3.tpl');
        module.exports.kd();
    }
};