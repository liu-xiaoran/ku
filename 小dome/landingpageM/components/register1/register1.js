...
module.exports = {
	kd:function() {
		...
		function countDown() {
			$("#get_identify").val("重新获取(" + time + ")").attr("disabled", "disabled").css({ "background": "#555", "color": "#fff" });
			time -= 1;
			if(time == 0) {
				$("#get_identify").val("重新获取").removeAttr("disabled").css({ "background": "#fdc30e", "color": "#ff4d4d" });
				time = 120;
				clearInterval(timer);
			}
		}
...
//						$(".downLoadcon").show();
//						$("html,body").css("height",$(".downLoadcon").height());
					} else {
						$(".prompt").html(data.errorMessage);
						me.show_prompt();
					}
    ...
    render: function (dom) {
        ...
        // 择
         var tpl = __inline('register1.handlebars');

	    // 模板数据
	    var data = {
	        // 使用__uri函数来定位任意工程文件，scrat构建之后，会
	        // 将其替换为发布地址，这样工程就不用关心部署相关问题了
	        bgimg1: window.remoteConfig.register1_bgimg1,
	        bgimg2: window.remoteConfig.register1_bgimg2,
	        bgcolor: window.remoteConfig.register1_bgcolor,
	        color1: window.remoteConfig.register1_color1,
	        color2: window.remoteConfig.register1_color2,
	        color3: window.remoteConfig.register1_color3,
	        color4: window.remoteConfig.register1_color4,
	        color5: window.remoteConfig.register1_color5
	    };
	    // 使用模板+数据得到html
	    dom.innerHTML = tpl(data);
//      dom.innerHTML = __inline('register1.tpl');
        module.exports.kd();
    }
};